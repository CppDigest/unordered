...
==== 带分配器的迭代器范围构造函数
```c++ template<class inputiterator=""> unordered_multiset(InputIterator f, InputIterator l, const allocator_type&amp; a); ```</class>
...
```c++ unordered_multiset(std::initializer_list<value_type> il, const allocator_type&amp; a); ```</value_type>
...
```c++ unordered_multiset(std::initializer_list<value_type> il, size_type n, const allocator_type&amp; a) ```</value_type>
...
```c++ unordered_multiset(std::initializer_list<value_type> il, size_type n, const hasher&amp; hf, const allocator_type&amp; a); ```</value_type>
...
==== 移动赋值
```c++unordered_multiset&amp; operator=(unordered_multiset&amp;&amp; other) noexcept(boost::allocator_traits<allocator>::is_always_equal::value &amp;&amp; boost::is_nothrow_move_assignable_v<hash> &amp;&amp; boost::is_nothrow_move_assignable_v<pred>);```移动赋值运算符。</pred></hash></allocator>
...
==== 初始化列表赋值
```c++ unordered_multiset&amp; operator=(std::initializer_list<value_type> il); ```</value_type>
...
==== 通过值擦除
```c++ size_type erase(const key_type&amp; k); template<class k=""> size_type erase(K&amp;&amp; x); ```</class>
...
==== 交换
```c++ void swap(unordered_multiset&amp;) noexcept(boost::allocator_traits<allocator>::is_always_equal::value &amp;&amp; boost::is_nothrow_swappable_v<hash> &amp;&amp; boost::is_nothrow_swappable_v<pred>); ```</pred></hash></allocator>
...
`template<class k="">` 重载仅在 `Hash::is_transparent` 和 `Pred::is_transparent` 是有效的成员 typedef 时参与重载决议。库假定 `Hash` 可同时使用 `K` 和 `Key` 调用，且 `Pred` 是透明的。这支持了异构查找，从而避免了实例化 `Key` 类型对象的开销。</class>
